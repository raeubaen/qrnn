...
python train_preshower.py -s ${ispl} -n ${nEvt}

python train_preshower_mc.py -r yes -s ${ispl}  -n ${nEvt}

python train_SS.py -d data -e ${EBEE} -n ${nEvt} -i ${SLURM_ARRAY_TASK_ID} -s ${ispl}
python train_SS_mc.py -e ${EBEE} -n ${nEvt} -r yes -s ${ispl}
